...
ifeq ($(TARGET_OS),darwin)
 ...
 LIBRARIES += -lGL -lGLU /usr/local/cuda/samples/common/lib/darwin/libGLEW.a
 ALL_LDFLAGS += -Xlinker -framework -Xlinker GLUT
else
 LIBRARIES += -L/usr/local/cuda/samples/common/lib/$(TARGET_OS)/$(TARGET_ARCH) $(GLLINK)
 LIBRARIES += -lGL -lGLU -lX11 -lglut -lGLEW -lrt -lpng
endif
...
julia_kernel.o: julia_kernel.cu
	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

JuliaExplorer.o:main_gl.cu
	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<


JuliaExplorer: cudaUtilities.o julia_kernel.o JuliaExplorer.o
