                      ...
                      float  ctr)
{
   ...
   float denom = (float)sqrt(2.0*M_PI*sigmaSq);
   float dist;
   for(int i=0; i<nPixels; i++)
   {
      dist = (float)i - pxCtr;
      targPtr[i] = expf(-0.5f * dist * dist / sigmaSq) / denom;
   }
}
                      ...
                      float  ctrCol)
{
   ...
   float denom = (float)(2.0*M_PI*sigmaRow*sigmaCol);
   for(int r=0; r<nPixelsRow; r++)
   {
      ...
      for(int c=0; c<nPixelsCol; c++)
      {
         ...
         targPtr[r*nPixelsCol+c] = expf(-0.5f*(distRowSqNorm + distColSqNorm)) / denom;
      }
   }
}
                                     ...
                                     int    diameter)
{
   ...
   for(int r=0; r<diameter; r++)
   {
      ...
      for(int c=0; c<diameter; c++)
      {
         ...
         float secondTerm = expf(-0.5f * (drSq + dcSq) / sigmaSq);
